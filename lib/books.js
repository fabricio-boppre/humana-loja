...
export async function getBooks(page, totalItensPerPage, formatsToFilterArray, conditionsToFilterArray, categoriesToFilterArray,priceRangesToFilterArray,order, currentSearchString) {
	...
	if (priceRangesToFilterArray.length > 0) {
		...
		if (priceRangesToFilterArray.indexOf(bookPriceRanges.priceRange31to60Id) !== -1) {
			priceRangeFilters.push('((price >= 31) && (price <= 60))')
		}
		if (priceRangesToFilterArray.indexOf(bookPriceRanges.priceRange61to90Id) !== -1) {
			priceRangeFilters.push('((price >= 61) && (price <= 90))')
		}
		if (priceRangesToFilterArray.indexOf(bookPriceRanges.priceRange91onwardsId) !== -1) {
