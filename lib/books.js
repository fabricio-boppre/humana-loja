...
export async function getBookCategories() {
	...
	const bookCategoriesQuery = '*[_type == "book_category"]{name,slug}|order(name asc)' 
...
export async function getBooks(page, totalItensPerPage, formatsToFilterArray, conditionsToFilterArray, categoriesToFilterArray,priceRangesToFilterArray) {
	...
	// - Now we check if there are categories filters; if yes, then we add them to the string:
	if (categoriesToFilterArray.length > 0) {
		const categoriesToFilterString = '["' + categoriesToFilterArray.join('","') + '"]'
		QueryFilters = QueryFilters + ' && (categories[]->slug)[@ in ' + categoriesToFilterString + ']'
	}
	// - Now we check if there are price ranges filters; if yes, then we add them to the string:
	// - We should apply the range(s) in the normal price or the discount price, if it exists.
	if (priceRangesToFilterArray.length > 0) {
		var priceRangeFilters = []
		if (priceRangesToFilterArray.indexOf('ate30') !== -1) {
			priceRangeFilters.push('((!defined(price_discount) && price <= 30) || (defined(price_discount) && price_discount <= 30))')
		}
		if (priceRangesToFilterArray.indexOf('31a60') !== -1) {
			priceRangeFilters.push('(((!defined(price_discount) && price >= 31) || (defined(price_discount) && price_discount >= 31)) && ((!defined(price_discount) && price <= 60) || (defined(price_discount) && price_discount <= 60)))')
		}
		if (priceRangesToFilterArray.indexOf('61a90') !== -1) {
			priceRangeFilters.push('(((!defined(price_discount) && price >= 61) || (defined(price_discount) && price_discount >= 61)) && ((!defined(price_discount) && price <= 90) || (defined(price_discount) && price_discount <= 90)))')
		}
		if (priceRangesToFilterArray.indexOf('maisde90') !== -1) {
			priceRangeFilters.push('((!defined(price_discount) && price > 90) || (defined(price_discount) && price_discount > 90))')
		}
